...
export function bindCardEvents(root, onDelete = (id) => deleteItem(id)) {
    root.querySelectorAll(".check").forEach((c) => {
        c.onchange = () => {
            const id = c.closest(".card").dataset.id;
            toggleDone(id);
        };
    });
    root.querySelectorAll(".del").forEach((d) => {
        d.onclick = () => {
