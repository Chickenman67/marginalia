...
export function fireNotifications(items) {
    if (!notificationsAllowed())
        return;
    for (const i of dueItems(items)) {
        if (notified.has(i.id))
            continue;
        notified.add(i.id);
        new Notification("Marginalia", { body: i.title });
    }
}
export function resetNotified(ids) {
